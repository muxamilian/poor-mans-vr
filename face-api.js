(function (global, factory) {
  ...
  ***************************************************************************** */
  ...
  var DrawFaceLandmarksMax = /** @class */ (function () {
    function DrawFaceLandmarksMax(faceLandmarks, options) {
        if (options === void 0) { options = {}; }
        this.faceLandmarks = faceLandmarks;
        this.options = new DrawFaceLandmarksOptions(options);
    }
    DrawFaceLandmarksMax.prototype.draw = function (canvasArg, dims) {
        var left_eye = this.faceLandmarks.positions.slice(36, 42);
        var right_eye = this.faceLandmarks.positions.slice(42, 48);
        var concatenated = left_eye.concat(right_eye);
        ...
        center_point.x /= concatenated.length;
        center_point.y /= concatenated.length;
        if (dims.width < dims.height) {
            var width_angle = 45;    
            var height_angle = width_angle*dims.height/dims.width;
        } else {
            var height_angle = 45;
            var width_angle = height_angle*dims.width/dims.height; 
        }
        var x_pos = center_point.x / dims.width;
        var y_pos = center_point.y / dims.height;
        var xdeg = x_pos * width_angle - width_angle/2;
        var ydeg = y_pos * height_angle - height_angle/2;
        var xdeg_current = xdeg;
        var ydeg_current = ydeg;
        if (window.xdeg_last === undefined) {
            window.last_time = Date.now();
            window.avg_delta = 0;
            window.xdeg_last = xdeg;
            window.ydeg_last = ydeg;
            window.xdeg_vector = 0;
            window.ydeg_vector = 0;
        } else {
            const new_time = Date.now();
            const diff = new_time - window.last_time;
            window.avg_delta = window.tradeoff * diff + (1-window.tradeoff) * window.avg_delta;
            window.xdeg_vector = window.tradeoff * (xdeg - window.xdeg_last) + (1-window.tradeoff) * xdeg_vector;
            window.ydeg_vector = window.tradeoff * (ydeg - window.ydeg_last) + (1-window.tradeoff) * ydeg_vector;
            xdeg_current = window.xdeg_last + window.xdeg_vector*diff/window.avg_delta;
            ydeg_current = window.ydeg_last + window.ydeg_vector*diff/window.avg_delta;
            window.xdeg_last = xdeg_current;
            window.ydeg_last = ydeg_current;
            window.last_time = new_time;
        }
        
        for (let i = 0; i < window.actual_videos.length; i++) {
            var window_width = window.video_bounding_rects[i].width;
            var window_height = window.video_bounding_rects[i].height;
            // var content_width = window.videos_to_change[i].scrollWidth;
            // var content_height = window.videos_to_change[i].scrollHeight;
            var content_width = window.videos_to_change_widths[i];
            var content_height = window.videos_to_change_heights[i];
            var video_to_change_width_angle = 60;
            var video_to_change_height_angle = video_to_change_width_angle*content_height/content_width;    
            
            var margin_left = (-content_width/2+window_width/2 - xdeg_current / video_to_change_width_angle * content_width);
            var margin_right = (-content_height/2+window_height/2 + ydeg_current / video_to_change_height_angle * content_height);
            // console.log(x_pos, margin_left, -content_width/2+window_width/2, xdeg_current);
            // console.log(content_width, window_width, -content_width/2+window_width/2);
            // margin_left = 0;
            // margin_right = 0;
            window.actual_videos[i].style.marginLeft = margin_left.toString()+'px';
            window.actual_videos[i].style.marginTop = margin_right.toString()+'px';
        }
    };
    return DrawFaceLandmarksMax;
}());
  ...
  function drawFaceLandmarksMax(canvasArg, faceLandmarks, dims) {
    var faceLandmarksArray = Array.isArray(faceLandmarks) ? faceLandmarks : [faceLandmarks];
    // console.log('faceLandmarksArray', JSON.stringify(faceLandmarksArray))
    faceLandmarksArray.forEach(function (f) {
        var landmarks = f instanceof FaceLandmarks
            ? f
            : (isWithFaceLandmarks(f) ? f.landmarks : undefined);
        ...
        new DrawFaceLandmarksMax(landmarks).draw(canvasArg, dims);
    });
}
  ...
  var index$1 = /*#__PURE__*/Object.freeze({
    ...
    drawFaceLandmarks: drawFaceLandmarks,
    drawFaceLandmarksMax: drawFaceLandmarksMax,
